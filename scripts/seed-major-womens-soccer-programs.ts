/**
 * Seed major Women's Soccer programs across every NCAA / NAIA / NJCAA tier.
 *
 * Conservative + idempotent (same contract as seed-major-football-programs.ts
 * and seed-major-womens-basketball-programs.ts):
 *   - University rows that already exist (matched by exact name OR slug) are
 *     reused — never overwritten.
 *   - Women's Soccer School rows that already exist for that university
 *     are skipped — we never create a duplicate (universityId, "Women's Soccer").
 ...
 *
 * Sport: "Women's Soccer" (Women).
 *
 * Notes specific to women's soccer:
 *   - Not every D1 school fields WSoc. Most SWAC, some MEAC, and a couple of
 *     football-only programs lack a varsity women's soccer team. This list
 *     only includes schools that *do* field WSoc as of 2025-2026.
 *   - WSoc conferences sometimes differ from football conferences (e.g.
 *     UConn football = Independent, WSoc = Big East).
 *
 * Usage:
 *   npm run seed:womens-soccer-major
 *
 * Add new schools at the bottom of WSOC_PROGRAMS — keep grouped by tier
...
const WSOC_PROGRAMS: WSocSeed[] = [
  ...
  { universityName: "United States Naval Academy", state: "MD", level: Division.D1, conference: "American Athletic Conference", tierLabel: "D1 / AAC" },
  ...
  { universityName: "University of Massachusetts Amherst", state: "MA", level: Division.D1, conference: "Mid-American Conference", tierLabel: "D1 / MAC (football)" },
  ...
  // --- D1 SWAC (only schools that field WSoc) ------------------------------
  ...
  // --- D1 MEAC (only schools that field WSoc) ------------------------------
  ...
  { universityName: "North Carolina A&T State University", state: "NC", level: Division.D1, conference: "Coastal Athletic Association", tierLabel: "D1 / CAA" },
...
async function findOrCreateUniversity(seed: WSocSeed): Promise<{ id: string; created: boolean }> {
...
async function findOrCreateWSocProgram(
  universityId: string,
  seed: WSocSeed
...
async function main() {
  console.log(`⚽  Seeding ${WSOC_PROGRAMS.length} major Women's Soccer programs…\n`);
  ...
  console.log(`Women's Soccer created:  ${stats.programsCreated}`);
  console.log(`Women's Soccer skipped:  ${stats.programsSkipped} (already existed)`);
